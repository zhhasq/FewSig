...
import models.model_sstsc.utils.datasets as ds
from models.model_sstsc.dataloader.TSC_data_loader import TSC_data_loader
...
class MultiUCR2018_WS(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            img_transformed = self.transform(img.copy())
            img_list.append(self.totensor_transform(img_transformed))
            # target_list.append(target.copy())
            # target_list.append(target)
            # target_list.append(self.totensor_transform(target))
            img_transformed_w=self.transform_w(img_transformed.copy())
            # img_transformed_w=img
