...
/***********************************************************************/
/***********************************************************************/
//c++ -O3 -Wall -shared -std=c++11 -fPIC $(python3 -m pybind11 --includes) UCR_DTW_py.cpp -o dtw_ucr$(python3-config --extension-suffix)
...
/// Main Function
double dtw_ucr_main(double* inputTS, double* q_input, long inputTSLength, int m, int r, long* bsf_index_result)
{
...
//    fclose(qp);
    ...
    while(!done)
    {
        ...
        while(ep<EPOCH) {
           ...
           //     break;
            //buffer[ep] = d;
            if (inputTSCount == inputTSLength) {
                break;
            }
            buffer[ep] = inputTS[inputTSCount];
            inputTSCount++;
            ep++;
        }
...
//                fprintf(stderr,".");
    ...
    bsf_index_result[0] = loc;
    return sqrt(bsf);
}

py::array_t<double> dtw_ucr(py::array_t<double>& x, py::array_t<double>& y, int w, py::array_t<long>& bsf_index) {

    py::buffer_info x_buffer = x.request();
    py::buffer_info y_buffer = y.request();
    py::buffer_info bsf_index_buffer = bsf_index.request();

    auto result_dist = py::array_t<double>(1);
    py::buffer_info result_dist_buff = result_dist.request();
    double *result_dist_ptr = (double *)result_dist_buff.ptr;

    double dist = dtw_ucr_main((double*)x_buffer.ptr, (double*)y_buffer.ptr, x_buffer.shape[0], y_buffer.shape[0], w, (long*)bsf_index_buffer.ptr);
    //result[0] is dtw distance
    result_dist_ptr[0] = dist;
    return result_dist;
}


PYBIND11_MODULE(dtw_ucr, m) {
    m.doc() = "pybind11 example plugin"; // optional module docstring
    m.def("dtw_ucr", &dtw_ucr);
}