...
import pickle
from models.model_sstsc.utils.helper import get_df_all
...
def load_cwru(opt):
    ...
    # data0, raw_labels = load_bearing_raw(dataset_path_, opt.dataset_name)
    # data16 = data0.astype(np.float16)
    # raw_labels = np.squeeze(raw_labels)
    #
    # with open('{}/{}/{}_data.pickle'.format(dataset_path, dataset_name, dataset_name), 'wb') as handle1:
    #     pickle.dump(data16, handle1)
    # with open('{}/{}/{}_label.pickle'.format(dataset_path, dataset_name, dataset_name), 'wb') as handle2:
    #     pickle.dump(raw_labels, handle2)
